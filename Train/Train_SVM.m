...
load('CRSPAM_features_H0_H1.mat');
...
model_2C_CRSPAM = svmtrain(trainLabel, trainData, ['-c ' num2str(bestc) ' -g ' num2str(bestg) ' -b 0']);
...
% Test
[predict_label, accuracy, decision_function_LM] = svmpredict(testLabel, testData, model_2C_CRSPAM, ' -b 0');
...
figure;
scatter(1:1997,decision_function_LM(1:1997,:),'filled'); xlabel('Sample Count'); ylabel('Decision Function');
hold on
%Scatter PLot for Mal
scatter(1:1997,decision_function_LM(1998:3994,:),'filled'); xlabel('Sample Count'); ylabel('Decision Function'); 
...
% % stepS = 0:0.005:1;
 ...
 figure(),plot(FP,TP,'*-');xlabel('False Positive'); ylabel('True Positive');title(sprintf('AUC: %.2f',abs(trapz(FP,TP))));

% % % %---------------------------------------------------------------------
% % % %                       Save The Results
% % % %---------------------------------------------------------------------
 
save(['Results_AWARE_' Proc_Type1 '_.mat'],'model_2C_CRSPAM','H0','H1','malacious1', 'data_H0_matrix', ...
    'data_malacious1_matrix1', 'decision_function_LM','accuracy','Error_Probability','predict_label','AUC','TP','FP', ...
    'Ntest_H0','Ntest_H1', 'cv_idx_H0', 'cv_idx_H1', 'tr_idx_H0', 'tr_idx_H1', 'te_idx_H0', 'te_idx_H1')
decision_function_LM_CRSPAM=decision_function_LM;
save('decision_function_LM_CRSPAM')
save('model_2C_CRSPAM')
% % % 
% % % 
t=toc;
